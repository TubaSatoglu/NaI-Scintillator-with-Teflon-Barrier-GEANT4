...
G4bool PMSensitiveDetector::ProcessHits(G4Step* step, G4TouchableHistory*) {
    G4Track* track = step->GetTrack();
        ...
        // Yalnızca optik fotonları kontrol ediyoruz.
        if (track->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) {
            fTotalOpticalPhotons++;

        // Hangi hacimde olduğuna göre sayım yapalım
        G4String volName = step->GetPreStepPoint()->GetPhysicalVolume()->GetName();

        if (volName == "ScintillatorPhys") {
            fPhotonsAtScintillator++;
        }
        // Not: Metal (alüminyum) yüzeye çarpan fotonlar, boundary sürecinde handle edildiği için
        // bu kısım pek çağrılmayacaktır.
        if (volName == "AluminumPlate" || volName == "AluminumSurface" || volName == "AluminumPhys") {
            ...
            PMEventAction* eventAction = static_cast<PMEventAction*>(G4EventManager::GetEventManager()->GetUserEventAction());
            if (eventAction) {
